...
function get_search_terms( $search_query ) {
	...
	if ( ! ( '' == $search_query ) || empty( $search_query ) ) {
		$search_query = html_entity_decode( $search_query, ENT_QUOTES, 'UTF-8' );
	} // Re encode any entities

	// strip out any characters that sql might use
	...
	\OrionRush\DuplicateDetector\Helpers\write_log( "DD Search terms:" );
	\OrionRush\DuplicateDetector\Helpers\write_log( $s_array );
...
/**
 *
 * Adds single quotes to elements of a string
 *
 * @since .0.0.3
 * @author orionrush
 *
 ...
 * @param   string $post_types Restrict to listed post types, a single post type as a string or an array of post types.
...
function get_any_matches( $search_query, $post_id = '', $post_status = '', $post_types = '', $by_date = '0' ) {
	...
	$to_search = get_search_terms( $search_query );
	$sql     = sql_prepare( $to_search, $post_id, $post_status, $post_types, $by_date );
	$results = $wpdb->get_results( $sql );

	// Log these values if we're debugging
	\OrionRush\DuplicateDetector\Helpers\write_log( "DD SQL Query:" );
	\OrionRush\DuplicateDetector\Helpers\write_log( $sql );

	\OrionRush\DuplicateDetector\Helpers\write_log( "DD Results:" );
	\OrionRush\DuplicateDetector\Helpers\write_log( $results );
